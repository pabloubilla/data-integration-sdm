...
def run_single_source_experiment(
    *,
    ...
    print(f"[{source_name.upper()}] test avg AUC: {avg_auc:.4f}")
    ...
    with open(exp_dir / "metrics.json", "w") as f:
        json.dump(
            {
                "avg_auc": avg_auc,
                "aucs": {str(k): None if np.isnan(v) else float(v) for k, v in aucs.items()},
            },
            f,
            indent=2,
        )
...
def run_integrated_experiment(
    *,
    ...
    criterion = IntegratedLoss(
        source1_loss=DeepMaxEntLoss(),
        source2_loss=BernoulliFromLogRateLoss(),
        source1_weight=w_po,
        source2_weight=w_pa,
        bias_weight=0.0,
        clamp_source1=(-10, 10),
    ...
    wandb.log(
        {
            ...
            "test/avg_auc": avg_auc,
        }
    )

    print(f"[PO+PA integrated] val avg AUC : {val_avg_auc:.4f}")
    print(f"[PO+PA integrated] test avg AUC: {avg_auc:.4f}")
...
def main():
    ...
    output_dir = Path("outputs/single_source_baselines")
    ...
    # Optional: remove coordinates for pure environmental baseline.
    # covariates = [c for c in covariates if c not in ["x", "y"]]
    ...
    if run_pa:
        results.append(
            run_single_source_experiment(
                source_name="pa",
                X_train_df=X_pa_train,
                y_train_lists=y_pa_train,
                X_test_df=X_pa_test,
                y_test_lists=y_pa_test,
    ...
    if run_po:
        results.append(
            run_single_source_experiment(
                ...
                criterion_name="deepmaxent",
                project_name=project_name,
            )
        )

    # -------------------------
    # PO+PA integrated baseline
    # -------------------------
    if run_integrated:
        results.append(
            run_integrated_experiment(
                X_po_df=X_po,
                y_po_lists=y_po,
                X_pa_train_df=X_pa_train,
                y_pa_train_lists=y_pa_train,
                X_pa_test_df=X_pa_test,
                y_pa_test_lists=y_pa_test,
                covariates=covariates,
                num_classes=num_classes,
                output_dir=output_dir,
                seed=seed,
