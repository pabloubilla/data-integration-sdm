...
# TODO: Work on this, all the DataSet classes should be changed accordingly
import os
import random
...
def run_experiment(
    ...
    val_loader = DataLoader(val_ds, batch_size=batch_size_consolidated, shuffle=False, drop_last=False,
                            collate_fn=lambda b: collate_multilabel(b, num_classes=len(species)))
    ...
    def train_model_(
        ...
        if criterion == 'deepmaxent':
            loss_f = DeepMaxEntLoss(pos_weight=pos_weight_t)
        if criterion == 'bce':
    ...
    def train_popa_model(
        ...
        for epoch in tqdm.tqdm(range(1, epochs + 1), desc="Training epochs"):
            ...
            for (xb_po, yb_po), (xb_pa, yb_pa) in zip(po_loader, pa_loader):
                ...
                yb_pa_weighted = yb_pa * pos_weight_t
                ...
                loss_pa = criterion_pa(probs_pa, yb_pa_weighted)
                loss_po = criterion_po(outputs_po, yb_po)
