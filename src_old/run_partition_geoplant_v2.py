...
from src_old.load_data import build_paths_nceas, load_po_pa_nceas, load_po_pa_geoplant
...
# --- your models & loss
from src_old.models import BalancedBCELoss, DeepMaxEntModel, SDMWithBias, IntegratedLoss, DeepMaxEntLoss
import src_old.pa_split as pa_split
from src_old.model_training import smooth_targets_v3, device


SPLIT_NUMBER = 30  # How many splits
...
def run_experiment_popa(
    ...
    print('Batch for PO: ', batch_size_po)
    print('Batch for PA: ', batch_size_pa)
    po_ds = XYDataset(X_po_s[covs].values.astype(np.float32), Y_po[species].values.astype(np.float32))
    pa_ds = XYDataset(X_pa_tr_s[covs].values.astype(np.float32), Y_pa_tr[species].values.astype(np.float32))
    ...
    # po_loader = DataLoader(po_ds, batch_size=batch_size_po, shuffle=True, drop_last=True) #, sampler=sampler_po
    # pa_loader = DataLoader(pa_ds, batch_size=batch_size_pa, shuffle=True, drop_last=True) #, sampler=sampler_pa
    ...
    X_te_np = X_pa_te_s[covs].values.astype(np.float32)
    scores = predict(model, X_te_np, dev=device())
    aucs = per_species_auc(Y_pa_te[species], scores, species)
    avg_auc = np.nanmean(list(aucs.values()))
    ...
    model_path = os.path.join(output_dir, f"deepmaxent_DA_{region}{group}.pt")
    torch.save(model, model_path)
    return avg_auc, aucs, avg_auc_site, None
...
def main():
    ...
    data_folder = f"france_sparse_1_0.2"
    ...
    for region in ['france']:
        for group in ['plants']:
            ...
            X_po, Y_po, X_pa, Y_pa, species, covs = load_po_pa_geoplant(
                data_path, region, filter_species=True, min_occurrence=20, npz_file=True,
                subsample_po = 1, subsample_pa = .5
            )
            ...
            pa_splits, split_type_list = pa_split.partition_sweep_ranges(
                X_pa_s, Y_pa, covs_cluster=covs_xy, covs_distance=covs, K_clusters=30, select_subset=SPLIT_NUMBER, train_proportion=1/3, distance_metric='mahalanobis')
            ...
            for split_id, (X_pa_tr, X_pa_te, Y_pa_tr, Y_pa_te, d_metric) in enumerate(pa_splits):
                ...
                if RUN_POPA_INTEGRATION:
                    print("\n--- Running PO+PA with Integrated Loss experiment ---")
                    
                    covs_to_use = covs + (['PO'] if ADD_PO_VAR else [])

                    auc_integration, aucs_integration, model_integration, scaler_integration = run_experiment_popa(
                        name="PO_plus_PA_integration",
                        X_po_df=X_po_s,
                        Y_po=Y_po,
                        X_pa_tr_df=X_pa_tr,
                        Y_pa_tr=Y_pa_tr,
                    ...
                    print(f"[PO+PA Integration] Average AUC on PA_test: {auc_integration:.4f}")
                ...
                row_info = {
                    ...
                    "AUC_PO_plus_PA_integration": float(np.round(auc_integration, 4)) if RUN_POPA_INTEGRATION else np.nan,
                ...
                for sp in species_used:
                    detailed_row = {
                        ...
                        "AUC_PO_plus_PA_integration_sp": float(np.round(aucs_integration[sp], 4)) if RUN_POPA_INTEGRATION else np.nan,
